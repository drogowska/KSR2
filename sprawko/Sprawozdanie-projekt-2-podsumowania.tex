...
\begin{enumerate}
  ...
  \item importance\_same\_race
   ...
   \begin{equation}
    ...
    \begin{equation}
      \mu_{\text{znaczące}} = e^{-{(x-6)}^2}, dla x \in [5,7]
    \end{equation}

    \begin{equation}
      \mu_{\text{ważne}} = e^{-(\frac{x-9}{1.5})^2}, dla x \in [7,10]
    \end{equation}
...
Poniżej zostały przedstawione wzory i wykresy przedstwiające poszczególne kwantyfikatory lingwistyczne. \( \mu_z(x) \) oznacza wartość funkcji przynależności kwantyfikatora lingwistycznego \(z\) dla danego atrybutu zbioru danych zależnie od wartości x:
