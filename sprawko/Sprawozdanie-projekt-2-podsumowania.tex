...
Na podstawie danych zebranych w Tabeli nr. 4 i 5, na pierwszy plan wysuwają się wartości miar $T_4$, $T_9$, $T_{10}$, $T_{11}$, gdyż posiadają one we wszystkich, bądź w większości, uzyskanych wyników takie same wartości. 
...
zastosowania więcej niż jednego z wcześniej wspomnianych elementów podsumowania. Miarę $T_2$ należy interpretować jako stopnień precyzyjności sumaryzatora, lub też jego braku. Zgodnie z danymi zawartymi w tabeli, najlepszy rezultat tej metryki otrzymał sumaryzator \(\text{age equals teenager}\), na poziomie wartości 0.898, lecz to podsumowanie pomimo, dobrze dobranego sumaryzatora, 
